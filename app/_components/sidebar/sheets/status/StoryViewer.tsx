...
const StoryViewer = ({ user, onClose, onDeleteStory }: StoryViewerProps) => {
  const timerInMS = 4000; // duratiom of story in miliseconds
    ...
    if (timerRunning) {
      intervalId = setInterval(() => {
        ...
        const newProgress = Math.round(
          ((timerInMS - newTimeRemaining) / timerInMS) * 100
        );
  ...
  return (
    <>
      {/* Top bar */}
      <div className="fixed top-0 left-0 w-screen h-fit flex justify-center items-start z-[20] pb-3 bg-gradient-to-t from transparent to-black">
        <div className="flex flex-col w-fit pt-4 text-white">
          <Progress className="w-[500px] h-2" value={progress} />
        ...
        />
        <ArrowLeft
          className="absolute top-4 left-4 text-white text-sxl cursor-pointer"
          onClick={handleClose}
        />
        <X
      ...
      ></div>
      {/* background colour while image load */}
      <div
        style={{
