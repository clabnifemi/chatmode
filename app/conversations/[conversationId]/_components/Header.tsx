    ...
    return (
      <>
             ...
             {conversation.isGroup ?(
                ...
                // Code for chanel case
                <div className="flex w-full">
             <Avatar>
    <AvatarImage  src={conversation.profileImageUrl || undefined }/>
    <AvatarFallback>{conversation.name ? conversation.name.charAt(0).toUpperCase() : 'CN'}
    </AvatarFallback>
    </Avatar>
    <div className="flex flex-col ml-2">
  <div>{conversation.name}</div>
    ...
    {conversation.ownerId !== currentUserPrisma.id && (
        <Button disabled={disableFollowButton} className={`ml-auto mr-4 ${conversation.userIds.includes(currentUserPrisma.id) ? 'bg-white text-primary hover:bg-white border-primary border-2' : 'bg-primary text-white'}`}
